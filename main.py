...
from tensorflow.keras.callbacks import TensorBoard, ModelCheckpoint, ReduceLROnPlateau, LearningRateScheduler, EarlyStopping, LambdaCallback
...
parser.add_argument("--current_epoch",type=int, required=True,help='epoch number of latest weights file (0 if training from the start)')
...
checkpoint = ModelCheckpoint(os.path.join(log_dir, 'ep{epoch:03d}-loss{loss:.3f}-val_loss{val_loss:.3f}-previous_epoch'+str(current_epoch)+'.h5'),
                monitor='val_loss',
                verbose=1,
...
model_loss, location_loss, confidence_loss, class_loss = Lambda(yolo3_loss, name='yolo_loss',
            ...
            'label_smoothing': label_smoothing, 'elim_grid_sense': elim_grid_sense})(
        [*model.output, *y_true])
...
if retrain==True :
        model.summary()
        if weights is None:
                model.fit(data_generator(dataset[:num_train], batch_size, image_shape, anchors, num_classes, multi_anchor_assign=False),
                        steps_per_epoch=max(1, num_train//batch_size),
                        validation_data=data_generator(dataset[num_train:], batch_size, image_shape, anchors, num_classes, multi_anchor_assign=False),
